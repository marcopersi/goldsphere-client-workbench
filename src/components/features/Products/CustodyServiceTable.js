...
const CustodyServiceTable = ({ custodians, selectedCustodian, setSelectedCustodian, handleCustodyServiceChange, t }) => {
  const handleRadioChange = useCallback((id) => {
    setSelectedCustodian(id);
  }, [setSelectedCustodian]);

  const handleSelectChange = useCallback((custodianId, value) => {
    handleCustodyServiceChange(custodianId, value);
  }, [handleCustodyServiceChange]);
          ...
          <tr key={custodian.id}>
            <td>
              <input
                ...
                onChange={() => handleRadioChange(custodian.id)}
              />
            </td>
            <td>
              {custodian.id === 'home_delivery' ? (
                ...
                <select
                  ...
                  onChange={(e) => handleSelectChange(custodian.id, e.target.value)}
                >
